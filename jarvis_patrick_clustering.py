...
def jarvis_patrick(data, labels, params_dict):
    ...
    for i in range(n):
        ...
        if similarity >= s_min:
            computed_labels[i] = majority_label + 1
...
def find_best_hyperparameters(data, labels, k_options, s_min_options, num_trials):
    top_avg_ARI = -1
    optimal_k = None
    optimal_s_min = None

    for k_value in k_options:
        for s_min_value in s_min_options:
            total_ARI_score = 0
            for _ in range(num_trials):
                params_dict = {'k': k_value, 's_min': s_min_value}
                computed_labels, avg_ARI_score, _ = jarvis_patrick(data, labels, params_dict)
                total_ARI_score += avg_ARI_score

            avg_ARI = total_ARI_score / num_trials
            if avg_ARI > top_avg_ARI:
                top_avg_ARI = avg_ARI
                optimal_k = k_value
                optimal_s_min = s_min_value
...
def jarvis_patrick_clustering():
    ...
    best_k, best_s_min = hyperparameter_study( subset_data,subset_labels k_range, s_min_range, num_trials)
    ...
    for i in [0, 1, 2, 3, 4]:
        ...
        groups[i] = {"s_min": best_s_min, "k": best_k, "ARI": ari, "SSE": sse}
